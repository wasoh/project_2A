...
void drawObject(vector<Ball> theBalls, Mat &frame){
    ...
    for (int i = 0; i < theBalls.size(); i++){
        ...
        cv::putText(frame, intToString(theBalls.at(i).getXPos()) + " , " + intToString(theBalls.at(i).getYPos()), cv::Point(theBalls.at(i).getXPos(), theBalls.at(i).getYPos() + 20), 1, 1, Scalar(0, 255, 0));
...
int test()
{
    ...
    while (1){
        ...
        if (calibrationMode == true){
            ...
            cvtColor(cameraFeed, HSV, COLOR_BGR2HSV);
            inRange(HSV, vert.getHSVmin(), vert.getHSVmax(), threshold);
            morphOps(threshold);
            trackFilteredObject(vert,threshold, HSV, cameraFeed);

            cvtColor(cameraFeed, HSV, COLOR_BGR2HSV);
            inRange(HSV, rouge.getHSVmin(), rouge.getHSVmax(), threshold);
            morphOps(threshold);
            trackFilteredObject(blanc,threshold, HSV, cameraFeed);

            cvtColor(cameraFeed, HSV, COLOR_BGR2HSV);
            inRange(HSV, blanc.getHSVmin(), blanc.getHSVmax(), threshold);
            morphOps(threshold);
            trackFilteredObject(rouge,threshold, HSV, cameraFeed);
        }
